function updateClock() {
  ...
  let currentTime = hours + ":" + minutes + " " + suffix;
  $(".time").html(currentTime);
}

let getTopZIndex = function() {
  let allDivs = $('.window');
  let topZindex = 0;
  let topDivId;
  allDivs.each(function() {
    let currentZindex = parseInt($(this).css('z-index'));
...
let createProgram = function(id, title, imgUrl, url) {
  ...
  $("#start-bar-" + id).css('background-image', 'url(' + imgUrl + ')');
       ...
       let content = '<div class="window ui-widget" id="' + id + '">' +
          ...
          '<iframe id="' + id + '" width="100%" height="100%" src="' + url + '" frameborder="0" allowfullscreen></iframe>' +
          '</div>' +
          '</div>' +
          '</div>';
...
$(".desktop-item").draggable();

let isWindowMaximised = function(id) {
  let targetId = $("#" + id);
...
let closeProgram = function(id) {
  $("#start-bar-" + id).remove();
  $("#" + id).remove();
};

let minimiseProgram = function(id) {
  $("#start-bar" + id).toggleClass("focused");
};

let maximiseWindow = function(id) {
  let targetId = $("#" + id);
...
let unMaximiseWindow = function(id) {
  let targetId = $("#" + id);
...
$(document).ready(function() {
  ...
  //start up music
  let audio = new Audio('http://www.winhistory.de/more/winstart/ogg/win98.ogg');
  // audio.play();
  // toggle start menu 
  ...
  $(".startbar").on("click", ".program", function(event) {
    ...
    if ($("#" + windowId).is(':visible')) {
      ...
      let z = getTopZIndex();
      let windowZ = $("#" + windowId).css("z-index");
      console.log(windowZ);
  ...
  $("#menu").on("click", ".launch", function(event) {
    ...
    let targetId = $(this).data("launch");
    let title = $(this).data("title");
    let imgUrl = $(this).data("icon");
  ...
  $("#menu").on("click", ".redirect", function(event) {
  ...
  $(".startbar").on("click", ".launch", function(event) {
    console.log($(this).data("launch"));
    let targetId = $(this).data("launch");
    let title = $(this).data("title");
    let imgUrl = $(this).data("icon");
  ...
  $(".dropdown-item-more").each(function() {
    if ($(this).data("icon")) {
      let icon = $(this).data("icon");
      $(this).css('background-image', "url(" + icon + ")");
    } else {
